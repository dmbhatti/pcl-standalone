...
bool FilterPointCloud(typename pcl::PointCloud<T>::ConstPtr cloud,
  ...
  for (unsigned int idx = 0; idx < temp.size(); ++idx) {
    if (!std::isnan(temp.points.at(idx).x) && !std::isnan(temp.points.at(idx).y) && !std::isnan(temp.points.at(idx).z)) {
 ...
 * @param dist_epsilon Possible error in the distance from the camera to the plane.
...
bool SegmentPlane(typename pcl::PointCloud<T>::ConstPtr cloud,
  ...
  try {
    ...
    float desired_dist_from_orig = transform_data[11] - table_height; //(in meters) desired projected (onto plane) distance from sensor
    if (transform_data[11] < table_height) {
      PCL_ERROR("[SegmentPlane][WARNING] Sensor height below table height. Setting to find floors.\n");
    ...
    for (int i = 0; i < normals->points.size(); ++i) {
      ...
      if ((angle < angle_range) || (angle > M_PI - angle_range)) {
        ...
        dist_from_orig = -(cloud->points[i].x * desired_plane_norm[0] + cloud->points[i].y * desired_plane_norm[1] + cloud->points[i].z * desired_plane_norm[2]);
        //fprintf(stderr, "[SegmentPlane] dist from origin: %f m\n", dist_from_orig);
        if (dist_from_orig < desired_dist_from_orig + dist_epsilon && dist_from_orig > desired_dist_from_orig - dist_epsilon) {
    ...
    // Mandatory
    seg.setModelType(pcl::SACMODEL_NORMAL_PLANE); //SACMODEL_PERPENDICULAR_PLANE);//SACMODEL_NORMAL_PLANE);SACMODEL_PLANE);
...
bool SegmentPlane2(typename pcl::PointCloud<PointT>::ConstPtr cloud,
        pcl::PointIndices::ConstPtr indices,
        pcl::PointIndices::Ptr plane_indices,
        pcl::PointIndices::Ptr objects_indices,
        pcl::ModelCoefficients::Ptr coefficients,
 ...
 */
template <class PointT>
bool filterPointsOnPlane(typename pcl::PointCloud<PointT>::ConstPtr cloud,
        pcl::PointIndices::ConstPtr indices_plane,
        pcl::PointIndices::ConstPtr indices_objects,
        pcl::ModelCoefficients::ConstPtr coeffs_plane,
        pcl::PointIndices::Ptr indices_real_objects, boost::mutex* mutexChull,
        float height = 0.35) {
  //TODO: modify this method to use pcl::CropHull
  //TODO: modify this to use ExtractPolygonalPrismData
  try {
    ...
    // Project the model inliers onto plane to make finding the convex hull easier
    ...
    // if dot product is negative than flip the normal
    ...
    //    // Project entire cloud onto plane to check if points are inside convex hull
    //    //i'm assuming the order of projected points is the same as original cloud
    //    typename pcl::PointCloud<PointT>::Ptr cloud_projected(new pcl::PointCloud<PointT> ());
    //    proj.setModelType(pcl::SACMODEL_PLANE);
    //    //proj.setInputCloud(cloudToFilter);
    //    proj.setModelCoefficients(coeffs_plane);
    ...
    //    //            viewer = boost::shared_ptr<pcl::visualization::PCLVisualizer > (new pcl::visualization::PCLVisualizer("filterPointsOnPlane"));
    //    //            viewer->initCameraParameters();
    //    //            viewer->addCoordinateSystem(0.1);
    //    //        }
    //    //
    //    //        pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> handler_cloudToFilter(cloudToFilter, 0, 255, 0);
    //    //        viewer->addPointCloud(cloudToFilter, handler_cloudToFilter, "capture");
    //    //        while (!viewer->wasStopped()) {
    //    //            viewer->spinOnce();
    //    //        }
    //    //        viewer->resetStoppedFlag();
    //    //fprintf(stderr, "[filterPointsOnPlane] plane norm: %f %f %f\n", plane_norm[0], plane_norm[1], plane_norm[2]);
    ////fprintf(stderr, "[filterPointsOnPlane] plane coeffs: %f %f %f %f\n", 
    ////coeffs_plane->values[0],coeffs_plane->values[1],coeffs_plane->values[2],coeffs_plane->values[3]);
    //
    //    //printf("[filterPointsOnPlane] cloud size: %d projected size: %d\n", cloudToFilter->size(), cloud_projected->size());
    //    indices_real_objects->indices.clear();
    //    for (int i = 0; i < indices_objects->indices.size(); ++i) 
    //    {
    //       int idx = indices_objects->indices.at(i);
    //       point = cloud->points.at(idx);
    //       temp_cloud->clear();
    //       temp_cloud->push_back(point);
    //       cloud_projected->clear();
    //
    //       proj.setInputCloud(temp_cloud);
    //       pcl::PointIndices::Ptr temp_indice(new pcl::PointIndices());
    //       temp_indice->indices.clear();
    //       temp_indice->indices.push_back(0);
    //       proj.setIndices(temp_indice);
    //       proj.filter(*cloud_projected);
    //       projectedPoint = cloud_projected->points[0];
    //
    //       //check distance constraint
    //       Eigen::Vector3f pointVec(point.x - projectedPoint.x, point.y - projectedPoint.y, point.z - projectedPoint.z);
    //       float dist = std::sqrt(std::pow(pointVec[0], 2) + std::pow(pointVec[1], 2) + std::pow(pointVec[2], 2));
    //       //float dist2 = std::sqrt(std::pow(plane_norm[0], 2) + std::pow(plane_norm[1], 2) + std::pow(plane_norm[2], 2));
    //       pointVec.normalize();
    //       float dir = plane_norm.dot(pointVec);
    //       //            PointT projected_norm;
    //       //            projected_norm.x = projectedPoint.x + plane_norm[0];
    //       //            projected_norm.y = projectedPoint.y + plane_norm[1];
    //       //            projected_norm.z = projectedPoint.z + plane_norm[2];
    //       //            viewer->addLine<PointT, PointT > (projectedPoint, point, 255, 0, 0, "line1");
    //       //            viewer->addLine<PointT, PointT > (projectedPoint, projected_norm, 255, 255, 0, "line2");
    //       //            while (!viewer->wasStopped()) {
    //       //                viewer->spinOnce();
    //       //            }
    //       //            viewer->resetStoppedFlag();
    ...
    ////double pcl_dist = pcl::pointToPlaneDistanceSigned(point, coeffs_plane->values[0],coeffs_plane->values[1],coeffs_plane->values[2],coeffs_plane->values[3]);
    //       //fprintf(stderr, "[filterPointsOnPlane] %d, dist: %f dir: %f normdist: %f signeddist: %f point: (%f,%f,%f)\n", i, dist, dir, dist2, pcl_dist, point.x, point.y, point.z);
    //       if ((dist < height) && (dir > 0)) //within height and on correct side of plane
    //       {
    //         //check that projected point is inside hull
    //         if (pcl::isPointIn2DPolygon<PointT > (projectedPoint, *cloud_hull)) 
    //         {
    //           indices_real_objects->indices.push_back(idx);
    //         }
    //       }
    //     }
    //     
    ...
    //        viewer->resetStoppedFlag();
    //printf("[filterPointsOnPlane] output size: %d\n\n\n\n", output->size());

    //    pcl::CropHull<PointT> hullFilter;
    //    hullFilter.setHullCloud(cloud_hull);
    //    hullFilter.setDim(3);
    //    hullFilter.setInputCloud(cloudToFilter);
    //    hullFilter.filter(*output);
  } catch (...) {
    return (false);
  }
  return (true);
...
template <class PointT>
bool ComputePolygonMesh(typename pcl::PointCloud<PointT>::ConstPtr cloud, pcl::PolygonMesh::Ptr mesh) {
  try {
    ...
    bool useMLS = true;
    if (useMLS) {
      //temp struct used to filter out NaNs
      pcl::PointCloud<pcl::PointNormal>::Ptr mls_results(new pcl::PointCloud<pcl::PointNormal>);
      ...
      mls.setSearchRadius(0.05); //0.05); //0.3

      //Optional upsampling params
      //mls.setUpsamplingMethod(pcl::MovingLeastSquares<PointT, pcl::PointNormal>::VOXEL_GRID_DILATION);
      //mls.setDilationVoxelSize(0.05); //VOXEL_GRID_DILATION only
      //mls.setDilationIterations(1);   //VOXEL_GRID_DILATION only
      //mls.setPointDensity(500);     //RANDOM_UNIFORM_DENSITY only

      // Reconstruct
      mls.process(*mls_results);

      //filter out NaNs (for some reason GreedyProjectionTriangulation can't handle them)
      pcl::PointNormal tmpPoint;
      for (int i = 0; i < mls_results->size(); ++i) {
        tmpPoint = mls_results->points[i];
        if (!std::isnan(tmpPoint.x) && !std::isnan(tmpPoint.y) && !std::isnan(tmpPoint.z)
                && !std::isnan(tmpPoint.normal_x) && !std::isnan(tmpPoint.normal_y) && !std::isnan(tmpPoint.normal_z)) {
          points_with_normals->push_back(tmpPoint);
        }
      }
    } else {
      //kdtree
    ...
    if (points_with_normals->points.size() == 0) {
      printf("GreedyProjectionTriangulation failed. Original cloud size: %lu. Filtered cloud size: %lu\n", cloud->points.size(), points_with_normals->points.size());
      return false;
    }
    ...
    printf("GreedyProjectionTriangulation original cloud size: %lu. filtered cloud size: %lu\n", cloud->points.size(), points_with_normals->points.size());
