...
Capture::Capture (QString file) : cloudFromFile (new PointCloudT()) {
  ...
  if (pcl::io::loadPCDFile<PointT> (file.toStdString(), *cloudFromFile) == -1) {
    std::cerr << "[capture.cpp] Couldn't read file " << file.toStdString() << std::endl;
    return;
  }
}
...
void Capture::run () {
  if(!useFile) {
    if (!grabber_->isRunning()) {
      std::cout << "[capture.cpp] Starting new grabber thread..." << std::endl;
    ...
    else std::cerr << "[capture.cpp] Grabber already running!" << std::endl;
  }
  else if (cloudFromFile->points.size() > 0) {
    const CloudMessage::Ptr message(new CloudMessage);
    message->emitterName = "Camera";
    message->emitterID = instanceID;
    ...
    std::cerr << "[capture.cpp] Could not read cloud file" << std::endl;
  }
}

void Capture::stop() {
  if (grabber_->isRunning()) {
    std::cout << "[capture.cpp] Stopping grabber thread..." << std::endl;
    grabber_->stop();
  }
}
